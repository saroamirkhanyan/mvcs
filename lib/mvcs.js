...
class MVCS {
  ...
  async getFileSplitted({ head, filePath }) {
    ...
    while (queue.length) {
      ...
      const changes = await this.store.get(commit.changes[filePath]);
      if (changes) {
        for (const lineNumber in changes) {
          if (!lines[lineNumber] && lines[lineNumber] !== null) {
  ...
  async diff({ head, file }) {
    ...
    const changes = {};
    let i;
    for (i = 0; i < splittedNewFile.length; i++) {
  ...
  async *repoAllFilesIterator(headId) {
    ...
    let commit;
    for (let commitId = headId; commitId; commitId = commit.prev) {
      commit = await this.store.get(commitId);
      const { changes } = commit;
      for (const filePath in changes) {
        if (!trackedFilesPaths.includes(filePath)) {
          trackedFilesPaths.push(filePath);
          if (changes[filePath] !== null) yield filePath;
        }
      }
    }
  }
  ...
  async save({ repoId, filesIterator }) {
    ...
    for await (const file of filesIterator) {
      ...
      if (changedCount) {
        ...
        commit.changes[file.path] = await this.store.save(changes);
				console.log("SAVED", changedCount, "FILES");
      }
